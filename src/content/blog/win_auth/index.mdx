...
Hiya there! Welcome to the first post on my blog. I've decided to write an informative albeit lengthy post on Windows authentication and post-exploitation techniques. I've structured this and the subsequent subposts to be read sequentially. Though, if you're already familiar with certain sections, feel free to skip ahead! Happy reading, and (ethically!) hacking **;)**
...
{/* ============ WORK ON ============ */}
<Callout type="question" title="TL;DR: What are Authentication Packages?" collapsed>
`.dll` files that sit somewhere in your system the LSA uses to facilitate authentication methods during the logon process. They're loaded during runtime when needed.
...
```mermaid
graph LR
	A[Authentication Protocols] --> B2[NTLM] & B3[Kerberos]
